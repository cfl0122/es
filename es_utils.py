...
class ES_FUNC:
    def __init__(self, house_wall, read_line, cross_rate,
        ...
        self.max_fitness = self.dna_size * (self.dna_size - 1) // 2
    ...
    def get_fitness(self, DNA):
        ...
        fitness = distance_fitness # kyt: 适应度只算距离适应度，因为现在方案中只要有不在红线中的点就适应度为0，把是否在区域中做成了一个判断条件了
    ...
    def init_P(self):
        ...
        for i in range(self.dna_size):
            # 将点初始化在红线的中心位置附近
            x = float(self.read_line.centroid.x[0])  + float(np.random.normal(loc=0, scale=120))
            y = float(self.read_line.centroid.y[0])  + float(np.random.normal(loc=0, scale=120))
            DNA.append([x, y])
        return DNA
    ...
    def crossover(self, POP):
        ...
        """
        first = self.select(POP)[:int(self.pop_size * 0.1)]
        for parent in POP:
        ...
        POP[self.pop_size - int(self.pop_size * 0.1):] = first
        return POP
